...
class apiInterface:
    ...
    def __init__(self, cfg, payloadTemplate=None, timeout=120):
        self.host    = cfg["host"]
    ...
    def setupConn(self):
        ...
        self.token = tknobj["jwtToken"] if "jwtToken" in tknobj else tknobj["ubus_rpc_session"]
        self.header = {
    ...
    def maintainConnection(req):
        def wrapper(*args):
            ...
            while True:
                try:
                    rsp = req(*args)
                    if "Expired bearer token" in json.dumps(rsp):
                        args[0].setupConn()
                        rsp = req(*args)
                    return rsp
                except Exception as e:
                    if time.time() > startTime + args[0].timeout:
                        raise Exception(f"Unable to reestablish connection after {args[0].timeout} seconds of trying. With error: {e}")
                    else:
                        time.sleep(2.5)
        return wrapper
...
def TestEvent(eventType, eventSubtype, obj, apiSend, apiRecv):
    ...
    if smsLog["data"]:
        ...
        if (obj["rsp"]["text"] == "" or msg[:-8:] == expected[:-8:]) and sender == apiRecv.telnum:
            didpass = "Passed"
